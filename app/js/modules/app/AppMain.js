...
// Get rid of the ones with no date and add a javaScript date to the array.
    ...
    $scope.allData = $scope.getAllData()
...
AppMain.factory('AppControllerModel', function(){
    ...
    this.cleanData = function(data){
        var key = 0
        return _.forEach(_.filter(data, function(n){
            return n.date && n.date != undefined
        }), function(n){
            n.key = key;
            key++
            n.jsDate = stringToDate(n.date)
            return n})
    }

    this.getCleanData = function(){
        if(!this.dataClean ) this.dataClean = this.cleanData( this.rawData );
